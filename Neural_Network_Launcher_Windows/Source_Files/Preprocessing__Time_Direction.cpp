...
bool Preprocessing__Time_Direction(void)
{
    ...
    SetConsoleTitle(std::string(tmp_dataset_name + " - Preprocessing, Time direction").c_str());
#endif
    ...
    if(tmp_Neural_Network_Manager.Get__Dataset_Manager()->Time_Direction(0_T,
                                                                                                                  1_T,
                                                                                                                  ENUM_TYPE_INPUT::TYPE_INPUT_OUTPUT) == false)
    {
        PRINT_FORMAT("%s: %s: ERROR: An error has been triggered from the \"Time_Direction(%f, %f, %u)\" function. At line %d." NEW_LINE,
                                 MyEA::Time::Date_Time_Now().c_str(),
                                 __FUNCTION__,
                                 Cast_T(0_T),
                                 Cast_T(1_T),
    ...
    if(tmp_Neural_Network_Manager.Initialize_Path(tmp_dataset_name, tmp_dataset_name + "_TD") == false)
    {
        PRINT_FORMAT("%s: %s: ERROR: An error has been triggered from the \"Initialize_Directory(%s, %s_TD)\" function. At line %d." NEW_LINE,
                                 MyEA::Time::Date_Time_Now().c_str(),
                                 __FUNCTION__,
                                 tmp_dataset_name.c_str(),
                                 tmp_dataset_name.c_str(),
