...
bool Preprocessing__Shift_Entries(void)
{
    ...
    SetConsoleTitle(std::string(tmp_dataset_name + " - Preprocessing, Shift entrie(s)").c_str());
#endif
    ...
    if(tmp_Neural_Network_Manager.Get__Dataset_Manager()->Shift_Entries(1_zu, ENUM_TYPE_INPUT::TYPE_INPUT_OUTPUT) == false)
    {
        PRINT_FORMAT("%s: %s: ERROR: An error has been triggered from the \"Shift_Entries(%zu, %u)\" function. At line %d." NEW_LINE,
                                 MyEA::String::Get__Time().c_str(),
                                 __FUNCTION__,
                                 1_zu,
    ...
    if(tmp_Neural_Network_Manager.Initialize_Path(tmp_dataset_name, tmp_dataset_name + "_ShE") == false)
    {
        PRINT_FORMAT("%s: %s: ERROR: An error has been triggered from the \"Initialize_Directory(%s, %s_ShE)\" function. At line %d." NEW_LINE,
                                 MyEA::String::Get__Time().c_str(),
                                 __FUNCTION__,
                                 tmp_dataset_name.c_str(),
                                 tmp_dataset_name.c_str(),
