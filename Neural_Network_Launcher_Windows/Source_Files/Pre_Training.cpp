...
bool Pre_Training(class MyEA::Capturing::Shutdown &shutdown_module)
{
    ...
    if(tmp_Neural_Network_Manager.Set__Desired_Loss(MyEA::String::Cin_Real_Number<T_>(0.0,
                                                                                                                                         1.0,
                                                                                                                                         MyEA::Time::Date_Time_Now() + ": Desired loss: ")) == false)
    {
    ...
    PRINT_FORMAT("%s" NEW_LINE, MyEA::Time::Date_Time_Now().c_str());
    PRINT_FORMAT("%s: Number of epochs per pre-training level." NEW_LINE, MyEA::Time::Date_Time_Now().c_str());
    PRINT_FORMAT("%s:\tdefault=250'000." NEW_LINE, MyEA::Time::Date_Time_Now().c_str());

    for(size_t tmp_level(1_zu); tmp_level != (tmp_ptr_neural_network->total_layers - 3_zu) / 2_zu + 2_zu; ++tmp_level)
    { tmp_vector_epochs_per_pre_training_level.push_back(MyEA::String::Cin_Number<size_t>(1_zu, MyEA::Time::Date_Time_Now() + ": Maximum epochs, level " + std::to_string(tmp_level) + ": ")); }
    ...
    PRINT_FORMAT("%s: FORM: Initialize chart of type gradient with 1 serie." NEW_LINE, MyEA::Time::Date_Time_Now().c_str());
    ...
    PRINT_FORMAT("%s" NEW_LINE, MyEA::Time::Date_Time_Now().c_str());
    bool const tmp_dataset_plot(MyEA::String::Accept(MyEA::Time::Date_Time_Now() + ": Do you want to plot the dataset?"));
