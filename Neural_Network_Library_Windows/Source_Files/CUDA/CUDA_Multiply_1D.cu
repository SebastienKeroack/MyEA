...
namespace Multiply
{
                                                                ...
                                                                T *const ptr_array_Z_received)
    {
        ...
        ptr_array_Z_received[stride_Z_received * tmp_thread_global_index] *= constant_received;
    }
                                                    ...
                                                    struct dim3 const *const ptr_dimension_block_received)
    {
        ...
        {
            LAUNCH_KERNEL_POINTER_1D(Multiply_Z_Y_1D<T>,
        ...
        {
            for(T const *const ptr_Z_end(ptr_array_Z_received + stride_Z_received * size_received); ptr_array_Z_received != ptr_Z_end; ptr_array_Z_received += stride_Z_received)
                                                    ...
                                                    struct dim3 const *const ptr_dimension_block_received)
    {
        if(USE_PARALLEL && size_received >= warpSize)
        {
            LAUNCH_KERNEL_POINTER_1D(Multiply_X_Y_1D<T>,
                                                              ptr_dimension_grid_received,
                                                              ptr_dimension_block_received,
                                                              0_zu,
                                                              size_received,
                                                              constant_received,
                                                              ptr_array_X_received)
        }
        else
        {
            for(T const *const ptr_X_end(ptr_array_X_received + size_received); ptr_array_X_received != ptr_X_end; ++ptr_array_X_received)
                                                                ...
                                                                T const *const ptr_array_X_received)
    {
        ...
        ptr_array_Z_received[tmp_thread_global_index * stride_Z_received] += constant_received * ptr_array_X_received[tmp_thread_global_index];
    }
                                                    ...
                                                    struct dim3 const *const ptr_dimension_block_received)
    {
        if(USE_PARALLEL && size_received >= warpSize)
        {
            LAUNCH_KERNEL_POINTER_1D(FMAC_Z_YX_1D<T>,
                                                              ptr_dimension_grid_received,
                                                              ptr_dimension_block_received,
                                                              0_zu,
                                                              size_received,
        ...
        {
            for(T const *const tmp_ptr_Z_end(ptr_array_Z_received + stride_Z_received * size_received); ptr_array_Z_received != tmp_ptr_Z_end; ptr_array_Z_received += stride_Z_received,
                                                                                                                                                                                                          ++ptr_array_X_received)
                                                                ...
                                                                T const *const ptr_array_Z_received)
    {
        ...
        ptr_array_X_received[tmp_thread_global_index] += constant_received * ptr_array_Z_received[tmp_thread_global_index * stride_Z_received];
    }
                                                    ...
                                                    struct dim3 const *const ptr_dimension_block_received)
    {
        if(USE_PARALLEL && size_received >= warpSize)
        {
            LAUNCH_KERNEL_POINTER_1D(FMAC_X_YZ_1D<T>,
                                                              ptr_dimension_grid_received,
                                                              ptr_dimension_block_received,
                                                              0_zu,
                                                              size_received,
        ...
        {
            for(T const *const tmp_ptr_X_end(ptr_array_X_received + size_received); ptr_array_X_received != tmp_ptr_X_end; ++ptr_array_X_received,
                                                                ...
                                                                T const *const ptr_array_inputs_X_received)
    {
        ...
        ptr_array_outputs_X_received[tmp_thread_global_index] += constant_received * ptr_array_inputs_X_received[tmp_thread_global_index];
    }
                                                    ...
                                                    struct dim3 const *const ptr_dimension_block_received)
    {
        if(USE_PARALLEL && size_received >= warpSize)
        {
            LAUNCH_KERNEL_POINTER_1D(FMAC_X_YX_1D<T>,
                                                              ptr_dimension_grid_received,
                                                              ptr_dimension_block_received,
                                                              0_zu,
                                                              size_received,
        ...
        {
            for(T const *const tmp_ptr_X_end(ptr_array_outputs_X_received + size_received); ptr_array_outputs_X_received != tmp_ptr_X_end; ++ptr_array_outputs_X_received,
                                                                                                                                                                                                        ++ptr_array_inputs_X_received)
            { *ptr_array_outputs_X_received += constant_received * *ptr_array_inputs_X_received; }
        }
    }

    template<typename T>
    __global__ void kernel__FMAC_X_YX_1D__atomic(T *const ptr_array_outputs_X_received,
                                                                              T const constant_received,
                                                                              T const *const ptr_array_inputs_X_received)
    {
        size_t const tmp_thread_global_index(blockIdx.x * blockDim.x + threadIdx.x);

        atomicAdd(ptr_array_outputs_X_received + tmp_thread_global_index, constant_received * ptr_array_inputs_X_received[tmp_thread_global_index]);
    }
    
    template<typename T>
    __global__ void kernel__FMAC_X_YX_1D__atomic(size_t const size_received,
                                                                              T *const ptr_array_outputs_X_received,
                                                                              T const constant_received,
                                                                              T const *const ptr_array_inputs_X_received)
    {
    ...
    template<typename T>
    __global__ void kernel_while__FMAC_X_YX_1D__atomic(size_t const size_received,
                                                                                       T *const ptr_array_outputs_X_received,
                                                                                       T const constant_received,
                                                                                       T const *const ptr_array_inputs_X_received)
    {
                                                                  ...
                                                                  struct dim3 const *const ptr_dimension_block_received)
    {
        ...
        else
        {
            for(T const *const tmp_ptr_X_end(ptr_array_outputs_X_received + size_received); ptr_array_outputs_X_received != tmp_ptr_X_end; ++ptr_array_outputs_X_received,
                                                                                                                                                                                                        ++ptr_array_inputs_X_received)
            { atomicAdd(ptr_array_outputs_X_received, constant_received * *ptr_array_inputs_X_received); }
        }
    }
}