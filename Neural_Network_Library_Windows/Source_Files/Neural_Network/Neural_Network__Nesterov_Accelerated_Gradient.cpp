...
void Neural_Network::Update_Parameter_Nesterov_Accelerated_Gradient__Loop(size_t const batch_size_received,
                                                                                                                     ...
                                                                                                                     size_t const end_index_received)
{
    ...
    if(tmp_weight_decay != 0_T)
    {
        for(tmp_connection_index = start_index_received; tmp_connection_index != end_index_received; ++tmp_connection_index)
        {
            ...
            tmp_ptr_array_parameters[tmp_connection_index] -= tmp_learning_momentum * tmp_delta_weigth - tmp_learning_rate * tmp_ptr_array_partial_derivative[tmp_connection_index] + tmp_ptr_array_connections_mask_rergularization[tmp_connection_index] * tmp_weight_decay * tmp_ptr_array_parameters[tmp_connection_index];
    ...
    else
    {
        for(tmp_connection_index = start_index_received; tmp_connection_index != end_index_received; ++tmp_connection_index)
        {
            ...
            tmp_ptr_array_parameters[tmp_connection_index] -= tmp_learning_momentum * tmp_delta_weigth - tmp_learning_rate * tmp_ptr_array_partial_derivative[tmp_connection_index]; // Gradient descent
...
void Neural_Network::Update_Parameter_Nesterov_Accelerated_Gradient__OpenMP(size_t const batch_size_received,
                                                                                                                           size_t const training_size_received,
                                                                                                                           size_t const start_index_received,
                                                                                                                           size_t const end_index_received)
{
