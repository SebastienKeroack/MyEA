...
void Neural_Network::Dropout_Zoneout__Block_Units__OpenMP(struct Layer *const ptr_layer_it_received)
{
    ...
    for(tmp_time_step__int = 0; tmp_time_step__int < tmp_number_recurrent_depth__int; ++tmp_time_step__int)
    {
        ...
        {
            if(this->ptr_array_Class_Generator_Bernoulli_Zoneout_State[tmp_thread_index__int].Generate_Bernoulli()) // Zoneout cell state.
            ...
            if(this->ptr_array_Class_Generator_Bernoulli_Zoneout_Hidden[tmp_thread_index__int].Generate_Bernoulli()) // Zoneout cell output.
