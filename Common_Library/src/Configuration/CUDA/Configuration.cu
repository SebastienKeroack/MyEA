...
void CUDA__Print__Device_Property(struct cudaDeviceProp const &ref_device_prop_received, int const index_device_received)
{
    ...
    if(index_device_received >= 0)
    {
        ...
        else { PRINT_FORMAT("%s: Concurrent copy and execution [Deprecated]: No" NEW_LINE, MyEA::String::Now_Format().c_str()); }
...
int CUDA__Maximum_Concurrent_Kernel(struct cudaDeviceProp const &ref_device_prop_received)
{
    ...
    switch(ref_device_prop_received.major)
    {
        ...
        case 3: // Kepler.
            ...
            else { PRINT_FORMAT("%s: Unknown minor device version." NEW_LINE, MyEA::String::Now_Format().c_str()); }
                break;
        case 5: // Maxwell.
        ...
        case 7: // Volta.
            if(ref_device_prop_received.minor == 0) { tmp_max_concurrent_kernel = 128; }
                ...
                break;
        default: PRINT_FORMAT("%s: Unknown major device version." NEW_LINE, MyEA::String::Now_Format().c_str());  break;
    }
...
bool CUDA__Input__Use__CUDA(int &ref_index_device_received, size_t &ref_maximum_allowable_memory_bytes_received)
{
    ...
    if(tmp_number_device != 0u)
    {
        ...
        for(unsigned int i(0u); i != tmp_number_device; ++i)
        {
            ...
            {
                tmp_use_CUDA = MyEA::String::Accept(MyEA::String::Now_Format() + ": Do you want to use CUDA with this card: " + std::string(tmp_cuda_device.name) + "?");
        ...
        if(tmp_use_CUDA)
        {
            ...
            PRINT_FORMAT("%s" NEW_LINE, MyEA::String::Now_Format().c_str());
            ...
            PRINT_FORMAT("%s" NEW_LINE, MyEA::String::Now_Format().c_str());
            PRINT_FORMAT("%s: Maximum allowable memory." NEW_LINE, MyEA::String::Now_Format().c_str());
            PRINT_FORMAT("%s:\tRange[1, %zu] MB(s)." NEW_LINE, MyEA::String::Now_Format().c_str(), tmp_memory_free_bytes / KILOBYTE / KILOBYTE);
            ...
            ref_maximum_allowable_memory_bytes_received = tmp_maximum_allowable_memory_bytes = MyEA::String::Cin_Number<size_t>(1_zu,
                                                                                                                                                                                                            ...
                                                                                                                                                                                                            MyEA::String::Now_Format() + ": Maximum allowable memory (MBs):");
