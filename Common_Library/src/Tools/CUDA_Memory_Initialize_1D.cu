...
namespace MyEA::Memory
{
    ...
    template<typename T>
    __global__ void kernel__Memory_Initialize_Index(size_t const size_received, T *const ptr_array_received)
    {
        size_t const tmp_thread_global_index(blockIdx.x * blockDim.x + threadIdx.x);
    ...
    template<typename T>
    __global__ void kernel_while__Memory_Initialize_Index(size_t const size_received, T *const ptr_array_received)
    {
    ...
    template<typename T>
    __global__ void kernel__Memory_Initialize_Index_Shift(size_t const size_received,
                                                                                   size_t const shift_received,
                                                                                   T *const ptr_array_received)
    {
        size_t const tmp_thread_global_index(blockIdx.x * blockDim.x + threadIdx.x);

        ptr_array_received[tmp_thread_global_index] = T((tmp_thread_global_index + shift_received) % size_received);
    }

    template<typename T>
    __global__ void kernel_if__Memory_Initialize_Index_Shift(size_t const size_received,
                                                                                      size_t const shift_received,
                                                                                      T *const ptr_array_received)
    {
        size_t const tmp_thread_global_index(blockIdx.x * blockDim.x + threadIdx.x);
    ...
    template<typename T>
    __global__ void kernel_while__Memory_Initialize_Index_Shift(size_t const size_received,
                                                                                            size_t const shift_received,
                                                                                            T *const ptr_array_received)
    {
        size_t const tmp_grid_stride(gridDim.x * blockDim.x);
                                                                        ...
                                                                        struct dim3 const *const ptr_dimension_block_received)
    {
        if(USE_PARALLEL && size_received >= warpSize)
        {
            if(ptr_dimension_grid_received->x * ptr_dimension_block_received->x < size_received)
            {
                kernel_while__Memory_Initialize_Index_Shift <<< *ptr_dimension_grid_received, *ptr_dimension_block_received >>> (size_received,
                                                                                                                                                                                          shift_received,
                                                                                                                                                                                          ptr_array_outputs_received);
            }
            else if(ptr_dimension_grid_received->x * ptr_dimension_block_received->x > size_received)
            {
                kernel_if__Memory_Initialize_Index_Shift <<< *ptr_dimension_grid_received, *ptr_dimension_block_received >>> (size_received,
                                                                                                                                                                                    shift_received,
                                                                                                                                                                                    ptr_array_outputs_received);
            }
            else
            {
                kernel__Memory_Initialize_Index_Shift <<< *ptr_dimension_grid_received, *ptr_dimension_block_received >>> (size_received,
                                                                                                                                                                                 shift_received,
                                                                                                                                                                                 ptr_array_outputs_received);
            }
        }
        ...
        {
            for(size_t i(0u); i != size_received; ++i)
            { ptr_array_outputs_received[i] = (i + shift_received) % size_received; }
        }
    }
    
    template<typename T>
    __global__ void kernel__Memory_Initialize_Index_Offset(size_t const offSet__received, T *const ptr_array_received)
    {
        size_t const tmp_thread_global_index(blockIdx.x * blockDim.x + threadIdx.x + offSet__received);

        ptr_array_received[tmp_thread_global_index] = T(tmp_thread_global_index);
    }

    template<typename T>
    __global__ void kernel__Memory_Initialize_Index_Offset(size_t const size_received,
    ...
    template<typename T>
    __global__ void kernel_while__Memory_Initialize_Index_Offset(size_t const size_received,
                                                                                              size_t const offSet__received,
                                                                                              T *const ptr_array_received)
    {
        size_t const tmp_grid_stride(gridDim.x * blockDim.x);
        size_t tmp_thread_global_index(blockIdx.x * blockDim.x + threadIdx.x + offSet__received);
    ...
    template<typename T>
    __global__ void kernel__Memory_Initialize_Index_Transposed(size_t const size_received,
                                                                                             ...
                                                                                             T *const ptr_array_outputs_received)
    {
        ...
        while(tmp_thread_global_index_block_y < width_received)
        {
            while(tmp_thread_global_index_block_x < width_received)
            {
                ...
                tmp_thread_global_index_offSet__x = tmp_thread_global_index_x + blockDim.x;

                tmp_thread_global_index_y = tmp_thread_global_index_block_y + tmp_thread_block_index_y;
                tmp_thread_global_index_offSet__y = tmp_thread_global_index_y + blockDim.y;

                if(tmp_thread_global_index_offSet__x < width_received && tmp_thread_global_index_offSet__y < width_received)
                {
                    ptr_array_outputs_received[tmp_thread_global_index_y * width_received + tmp_thread_global_index_x] = tmp_thread_global_index_x * width_received + tmp_thread_global_index_y;
                    ptr_array_outputs_received[tmp_thread_global_index_y * width_received + tmp_thread_global_index_offSet__x] = tmp_thread_global_index_offSet__x * width_received + tmp_thread_global_index_y;
                    ptr_array_outputs_received[tmp_thread_global_index_offSet__y * width_received + tmp_thread_global_index_x] = tmp_thread_global_index_x * width_received + tmp_thread_global_index_offSet__y;
                    ptr_array_outputs_received[tmp_thread_global_index_offSet__y * width_received + tmp_thread_global_index_offSet__x] = tmp_thread_global_index_offSet__x * width_received + tmp_thread_global_index_offSet__y;
                }
    ...
    __device__ void Memory_Initialize_Index_Transposed(size_t const size_received,
                                                                                 ...
                                                                                 struct dim3 const *const ptr_dimension_block_received)
    {
        size_t const tmp_width(static_cast<size_t>(floor(sqrt(static_cast<double>(size_received)))));

        if(USE_PARALLEL && tmp_width * tmp_width >= warpSize)
        {
            kernel__Memory_Initialize_Index_Transposed <<< *ptr_dimension_grid_received, *ptr_dimension_block_received >>> (tmp_width * tmp_width,
                                                                                                                                                                                       tmp_width,
                                                                                                                                                                                       ptr_array_outputs_received);
        }
        else
        {
            for(size_t row(0u),
                                   column(0u); column != tmp_width; ++column)
            {
                for(row = 0u; row != tmp_width; ++row)
                { ptr_array_outputs_received[column * tmp_width + row] = row * tmp_width + column; }
            }
        }
