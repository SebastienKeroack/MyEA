...
Shutdown_Block::Shutdown_Block(std::string const &ref_window_name_received, std::string const &ref_class_name_received) : _window_name(ref_window_name_received),
                                                                                                                                                                                          _class_name(ref_class_name_received)
{ }

BOOL WINAPI Shutdown_Block::WINAPI__ConsoleCtrlHandler(DWORD dwCtrlType_received)
{
...
void Shutdown_Block::ConsoleCtrlHandler(DWORD const dwCtrlType_received)
{
    if(this->_initialize)
    {
        if(dwCtrlType_received == CTRL_C_EVENT ||
...
ATOM Shutdown_Block::Register_Class(HINSTANCE HINSTANCE_received)
{
...
BOOL Shutdown_Block::Init_Instance(HINSTANCE HINSTANCE_received, int nCmdShow_received)
{
    if(this->_window_name.empty())
    {
        PRINT_FORMAT("%s: %s: ERROR: Window name is empty. At line %d." NEW_LINE,
    ...
    {
        PRINT_FORMAT("%s: %s: ERROR: Class name is empty. At line %d." NEW_LINE,
                                 MyEA::String::Get__Time().c_str(),
                                 __FUNCTION__,
                                 __LINE__);
    ...
    if(this->_HWND == NULL)
    {
        PRINT_FORMAT("%s: %s: ERROR: An error has been triggered from the \"CreateWindow()\" function. At line %d." NEW_LINE,
                                    MyEA::String::Get__Time().c_str(),
                                    __FUNCTION__,
                                    __LINE__);
    ...
    {
        PRINT_FORMAT("%s: %s: ERROR: An error has been triggered from the \"UpdateWindow()\" function. Code: %d. At line %d." NEW_LINE,
                                 MyEA::String::Get__Time().c_str(),
                                 __FUNCTION__,
                                 tmp_return_code,
                                 __LINE__);
...
bool Shutdown_Block::Create_Shutdown_Block(bool const use_ctrl_handler_received)
{
    if(this->_initialize == false)
    {
        ...
        {
            if(use_ctrl_handler_received && (tmp_return_code = SetConsoleCtrlHandler(static_cast<PHANDLER_ROUTINE>(Shutdown_Block::WINAPI__ConsoleCtrlHandler), TRUE)) == FALSE)
            {
                PRINT_FORMAT("%s: %s: ERROR: An error has been triggered from the \"SetConsoleCtrlHandler()\" function. Code: %d. At line %d." NEW_LINE,
                                         MyEA::String::Get__Time().c_str(),
                                         __FUNCTION__,
                                         tmp_return_code,
                                         __LINE__);
            ...
            {
                PRINT_FORMAT("%s: %s: ERROR: An error has been triggered from the \"GetModuleHandle(NULL)\" function. At line %d." NEW_LINE,
                                         MyEA::String::Get__Time().c_str(),
                                         __FUNCTION__,
                                         __LINE__);
            ...
            {
                PRINT_FORMAT("%s: %s: ERROR: An error has been triggered from the \"Init_Instance()\" function. Code: %d. At line %d." NEW_LINE,
                                         MyEA::String::Get__Time().c_str(),
                                         __FUNCTION__,
                                         tmp_return_code,
                                         __LINE__);
        ...
        else
        {
            if(this->_HWND == NULL)
            {
                PRINT_FORMAT("%s: %s: ERROR: HWD is a nullptr while HINSTANCE is not a nullptr. At line %d." NEW_LINE,
                                         MyEA::String::Get__Time().c_str(),
                                         __FUNCTION__,
                                         __LINE__);
...
bool Shutdown_Block::Peak_Message(void)
{
    if(this->_initialize && this->_on_shutdown == false)
    {
                                                                    ...
                                                                    PM_REMOVE)) != 0)
        {
            ...
            {
                PRINT_FORMAT("%s: %s: ERROR: An error has been triggered from the \"PeekMessage()\" function. Code: %d. At line %d." NEW_LINE,
                                         MyEA::String::Get__Time().c_str(),
                                         __FUNCTION__,
                                         tmp_return_code,
                                         __LINE__);
