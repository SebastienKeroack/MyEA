...
Shutdown_Capturing::Shutdown_Capturing(std::string const &ref_window_name_received, std::string const &ref_class_name_received) : _window_name(ref_window_name_received),
                                                                                                                                  _class_name(ref_class_name_received)
{ }

BOOL WINAPI Shutdown_Capturing::WINAPI__ConsoleCtrlHandler(DWORD dwCtrlType_received)
{
...
void Shutdown_Capturing::ConsoleCtrlHandler(DWORD const dwCtrlType_received)
{
    if(this->_initialize)
    {
        if(dwCtrlType_received == CTRL_C_EVENT ||
...
ATOM Shutdown_Capturing::Register_Class(HINSTANCE HINSTANCE_received)
{
...
BOOL Shutdown_Capturing::Init_Instance(HINSTANCE HINSTANCE_received, int nCmdShow_received)
{
...
bool Shutdown_Capturing::Create_Shutdown_Block(bool const use_ctrl_handler_received)
{
    if(this->_initialize == false)
    {
        ...
        {
            if(use_ctrl_handler_received && (tmp_return_code = SetConsoleCtrlHandler(static_cast<PHANDLER_ROUTINE>(Shutdown_Capturing::WINAPI__ConsoleCtrlHandler), TRUE)) == FALSE)
            {
